...
def usage():
    ...
    print("       or")
    print(
        "       mpirun -np {number of processes} python "
        + sys.argv[0]
        + ' [-n|--no-reparam]"'
    )
    print("")
    print("Options:")
    print(
        "         -n | --no-reparam       This flag can be used to skip the reparametrisation"
    )
    print(
        "                                 (e.g., in case one runs the filtering several times"
    )
    print(
        "                                 for one model that already has been reparametrised)."
    )
    print("")
...
def main(argv):
    ...
    for ilyr in range(1, nl + 1):
        ...
        if ilyr <= nl_UM_TZ:
            m_prime_s = np.zeros(np_UM_TZ)
            m_prime_p = np.zeros(np_UM_TZ)
            my_m_prime_s = np.zeros(np_UM_TZ)
            my_m_prime_p = np.zeros(np_UM_TZ)
        else:
            m_prime_s = np.zeros(np_LM)
            m_prime_p = np.zeros(np_LM)
            my_m_prime_s = np.zeros(np_LM)
            my_m_prime_p = np.zeros(np_LM)
        ...
        for ip in range(my_ib, my_ie + 1):
            ...
            my_m_prime_s[cri] = (
                my_m_prime_s[cri] + R_ij[ip] * slowness_perturbation_s[l_index][c_index]
            )
            my_m_prime_p[cri] = (
                my_m_prime_p[cri] + R_ij[ip] * slowness_perturbation_p[l_index][c_index]
            )

        # Now collect all contributions on the master proc
        comm.Reduce(
            [my_m_prime_s, MPI.DOUBLE], [m_prime_s, MPI.DOUBLE], op=MPI.SUM, root=0
        )
        comm.Reduce(
            [my_m_prime_p, MPI.DOUBLE], [m_prime_p, MPI.DOUBLE], op=MPI.SUM, root=0
        )
        ...
        if myrank == 0:
            ...
            m_prime_s = (
                m_prime_s * -v_1D_s[ilyr - 1]
            )  # dv = du*v_1D^2 => dv/v_1D = du*v_1D
            m_prime_p = (
                m_prime_p * -v_1D_p[ilyr - 1]
            )  # dv = du*v_1D^2 => dv/v_1D = du*v_1D
